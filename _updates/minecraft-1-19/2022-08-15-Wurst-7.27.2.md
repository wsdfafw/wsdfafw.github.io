...
- 修复了如果您的 Microsoft 帐户受到家长控制或禁止在线游戏，多人游戏按钮仍然显示为灰色。

- <del>修复了在启用 [NoChatReports](https://wiki.wurstclient.net/nochatreports) 时尝试在 Minehut 上聊天时出现的神秘错误消息。 (MC 1.19+)</del> **更新：** Minehut 现在已经解决了这个问题。 使用 NoChatReports 时，他们将不再踢你。

- 修正了很多有太多换行符的描述。

- 清理了 Wurst 过滤器设置背后的代码。

- 修复了各种“过滤器休眠”和“过滤器飞行”设置的不一致描述。

- 修正[AnchorAura](https://wiki.wurstclient.net/anchoraura) 和[CrystalAura](https://wiki.wurstclient.net/crystalaura) 过滤器设置描述中的一些错别字。
